...
export const ProjectProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateTask: ProjectContextType['updateTask'] = useCallback(
    (projectId, taskId, newStatus, newProgress) => {
  ...
  const addTask: ProjectContextType['addTask'] = useCallback(
    (projectId, newTask) => {
      setProjects((prevProjects) =>
        prevProjects.map((project) =>
          project.id === projectId
  ...
  const updateMilestoneProgress = useCallback(
    (projectId: string, milestoneId: string, newProgress: number) => {
      setProjects((prevProjects) =>
        prevProjects.map((project) =>
          project.id === projectId
            ? {
